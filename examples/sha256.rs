...
/// Toy circuit: prove knowledge of a SHA-256 preimage.
/// Instance: 32-byte hash digest (one field element per byte)
...
fn main() -> Result<()> {
    ...
    let formatted_instance = Sha256Preimage::format_instance(&instance).unwrap();
    let formatted_commited_instance = Sha256Preimage::format_committed_instances(&witness);
    create_proof(
        &params,
        &pk,
        &[circuit],
    ...
    let chips = &[SupportedChips::Sha256];
    cost_evaluation(
        &params,
        &vk,
        None,
