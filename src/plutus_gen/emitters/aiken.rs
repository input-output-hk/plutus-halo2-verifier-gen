...
use crate::plutus_gen::extraction::data::{
    CircuitRepresentation, CommitmentData, MsmOperations, OptimizedMSM, ProofExtractionSteps,
    Query, RotationDescription, ScalarOperation, constants::*,
};
    ...
    PCS: ExtractPCS,
{
    ...
    if PCS::pcs_type() == PCSType::GWC19 {
        ...
        let optimized_left: OptimizedMSM = left.flatten_msm().optimize_msm();
        let optimized_right: OptimizedMSM = right.flatten_msm().optimize_msm();

        let kzg_gwc19_msm = format!(
            "    let el = eval({})\n    let er = eval({})",
            AikenExpression::compile_expression(&optimized_left),
            AikenExpression::compile_expression(&optimized_right),
        );
