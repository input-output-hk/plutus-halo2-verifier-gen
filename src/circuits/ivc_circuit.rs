...
pub fn from_ivc(
    ...
    prev_state: F,
    prev_proof: Vec<u8>,
    prev_acc: Accumulator<S>,
) -> IvcCircuit {
    IvcCircuit {
        self_vk: (domain, cs, Value::known(vk.transcript_repr())),
        prev_state: Value::known(prev_state),
        prev_proof: Value::known(prev_proof),
        prev_acc: Value::known(prev_acc),
    }
}
...
mod tests {
    ...
    fn test_ivc_circuit() {
        ...
        let vk = keygen_vk_with_k(&kzg_params, &default_ivc_circuit, self_k).unwrap();
