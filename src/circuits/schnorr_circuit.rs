...
use midnight_curves::{
    Fq as JubjubBase, Fr as JubjubScalar, JubjubAffine, JubjubExtended as Jubjub, JubjubSubgroup,
};
use midnight_proofs::{
...
pub mod utils {
    ...
    pub fn verify(sig: &SchnorrSignature, pk: &SchnorrPK, m: Message) -> bool {
        let mut buff = [0u8; 64];
    ...
    pub fn get_coords(point: &JubjubSubgroup) -> (F, F) {
...
impl Relation for SchnorrExample {
    ...
    fn circuit(
        ...
        let sig_e = std_lib
            .jubjub()
            .scalar_from_le_bytes(layouter, &sig_e_bytes)?;
